Feature: [SUC:01-11] Create Applicant Portal Credential
  ...
  @SUC:01-11
  Scenario:UAT_TCS 03.01.2 To Verify the process of Validating Mandatory Details
    Given user clicks applicant submit button with fields blank
    Then submit button should not be clickable

  @SUC:01-11
  Scenario Outline:UAT_TCS 03.01.4 To verify the process of successfully creating portal credentials
    Given user inputs identification details
#    change nid after each use
      | FirstName     | Max        |
      | LastName      | Mende      |
      | DOB           | 01/01/1990 |
      | POB           | Lilongwe   |
...
#    change email after each run
      ...
      | Individual                 | NID Card            | max2@mailinator.com | Passw0rd@123 | C:\id_doc.png |
